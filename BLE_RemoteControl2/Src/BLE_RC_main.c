...
{  
  ...
  BSP_LED_Init_Team();
  BSP_LED_Off_Team(LL_GPIO_PIN_14);
  BSP_LED_Off_Team(LL_GPIO_PIN_14);
  ...
  extern uint8_t s;
  s = 0;
  float ths = 3.0; //trashhold
  extern uint8_t auto_switch;
  float critic = 45.0;
  extern float temperature_degC;
  uint8_t sw_state = 0;
  while(1) {
	extern uint32_t pinSwitch;
	extern uint32_t counter;
    ModulesTick();
    switch(s){
    case 1:
    	PRINTF("T %d ON.\n",pinSwitch);
    	BSP_LED_On_Team(pinSwitch);
    	sw_state = (pinSwitch == LL_GPIO_PIN_14 );
    	counter = 1;
    	s = 2;
    	break;
    case 2:
    	if(counter > 300000){
    		BSP_LED_Off_Team(pinSwitch);
    		PRINTF("T %d OFF.\n",pinSwitch);
    		counter = 1;
    		s = 0;
    		if(auto_switch)
    ...
    case 1:
    	if(!sw_state){
    		if(temperature_degC >= critic){
    			pinSwitch = LL_GPIO_PIN_14;
    			s = 1;
    			auto_switch = 2;
    		}
    	}
    	else {
    		if(temperature_degC < critic-ths){
				pinSwitch = LL_GPIO_PIN_15;
				s = 1;
				auto_switch = 2;
			}
    	}
...
void BSP_LED_Init_Team()
{
  /* Configure the LED pin and turn off it */
	...
	LL_GPIO_SetPinPull(GPIOB, LL_GPIO_PIN_14, BSP_LED_PULL_MODE);
    ...
    LL_GPIO_SetPinPull(GPIOB, LL_GPIO_PIN_15, BSP_LED_PULL_MODE);
  ...
  if(Led == BSP_LED1) {
    ...
    LL_GPIO_SetPinPull(BSP_LED1_GPIO_PORT, BSP_LED1_PIN, BSP_LED_PULL_MODE);
  }
    ...
    LL_GPIO_SetPinPull(BSP_LED2_GPIO_PORT, BSP_LED2_PIN, BSP_LED_PULL_MODE);
  }
    ...
    LL_GPIO_SetPinPull(BSP_LED3_GPIO_PORT, BSP_LED3_PIN, BSP_LED_PULL_MODE);
  }*/
...
void BSP_LED_On_Team(int Led)
  {
    if(Led == LL_GPIO_PIN_14) {
    	LL_GPIO_SetOutputPin(GPIOB, LL_GPIO_PIN_14);
    }
    else if(Led == LL_GPIO_PIN_15) {
